...
import { mockCustomers, mockOrders, mockAppointments, revenueData, serviceDistribution, formatVND } from '../../data/mockData';
...
function KpiCard({ label, value, sub, accent, icon }: { label: string; value: string | number; sub: string; accent: string; icon: string }) {
  return (
    <div className="rounded-2xl p-5" style={{ background: 'white', border: '1px solid var(--color-zinc-200)' }}>
      ...
      <div style={{ fontFamily: 'var(--font-display)', fontSize: 32, fontWeight: 800, color: accent, letterSpacing: '0.02em', lineHeight: 1 }}>{value}</div>
...
export default function DashboardPage() {
  const revenue12 = revenueData.reduce((s, d) => s + d.doanhThu, 0);
  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="mb-6">
        ...
        <p className="text-sm mt-1" style={{ color: 'var(--color-zinc-500)' }}>Tổng quan hệ thống — cập nhật theo thời gian thực</p>
      </div>

      {/* KPIs */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <KpiCard label="Tổng khách hàng" value={mockCustomers.length} sub="Đã đăng ký" accent="var(--color-red-700)" icon="👥" />
        <KpiCard label="Đơn hàng tháng này" value={mockOrders.length} sub={`${pendingOrders} chờ duyệt`} accent="#2563eb" icon="📦" />
        <KpiCard label="Lịch hẹn chờ duyệt" value={pendingAppts} sub="cần xử lý" accent="#d97706" icon="📅" />
        <KpiCard label="Doanh thu 7 tháng" value={formatVND(revenue12)} sub="Phụ tùng" accent="var(--color-success)" icon="💰" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-6">
        ...
        <div className="lg:col-span-2 rounded-2xl p-6" style={{ background: 'white', border: '1px solid var(--color-zinc-200)' }}>
          ...
          <p className="text-xs mb-5" style={{ color: 'var(--color-zinc-500)', fontFamily: 'var(--font-mono)' }}>T6 – T12/2024 · Đơn vị: VNĐ</p>
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={revenueData}>
              <CartesianGrid strokeDasharray="3 3" stroke="var(--color-zinc-100)" vertical={false} />
              <XAxis dataKey="thang" tick={{ fontSize: 12, fontFamily: 'var(--font-mono)', fill: 'var(--color-zinc-500)' }} axisLine={false} tickLine={false} />
              <YAxis tickFormatter={v => `${(v / 1000000).toFixed(0)}M`} tick={{ fontSize: 11, fontFamily: 'var(--font-mono)', fill: 'var(--color-zinc-400)' }} axisLine={false} tickLine={false} />
              <Tooltip
        ...
        <div className="rounded-2xl p-6" style={{ background: 'white', border: '1px solid var(--color-zinc-200)' }}>
          ...
          <p className="text-xs mb-4" style={{ color: 'var(--color-zinc-500)', fontFamily: 'var(--font-mono)' }}>Loại dịch vụ khách hay đặt</p>
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie data={serviceDistribution} cx="50%" cy="50%" outerRadius={90} dataKey="value" labelLine={false} label={PieLabel}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="rounded-2xl" style={{ background: 'white', border: '1px solid var(--color-zinc-200)' }}>
          <div className="px-5 py-4 border-b" style={{ borderColor: 'var(--color-zinc-200)' }}>
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 15, color: 'var(--color-zinc-900)', letterSpacing: '0.04em', textTransform: 'uppercase' }}>LỊCH HẸN HÔM NAY</div>
          </div>
